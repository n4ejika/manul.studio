(() => {
  ...
  if (!root || !core) return;
  const money = value => new Intl.NumberFormat(root.dataset.estimateLanguage, {style:'currency',currency:'USD',maximumFractionDigits:0}).format(value);
  ...
  const update = () => {
    ...
    set('monthly', core.calculateSeo({queries:value('queries'),pages:value('seoPages'),market:value('market')}, 'en').total);
    if (root.querySelector('[data-project-estimate]')) {
      const result = core.calculate({pages:value('pages'),services:value('services'),contours:value('contours')}, 'en');
      set('development', result.developmentTotal);
      set('launch', result.launchTotal);
      set('developmentIntro', core.introductoryPrice(result.developmentTotal, 'en'));
